...
class Animator:
    def __init__(
        ...
        self.config_axes = lambda: (
            ...
            or self.axes[0].set_ylabel(ylabel)
            or self.axes[0].set_xlim(xlim)
            or self.axes[0].set_ylim(ylim)
            or self.axes[0].legend(legend)
            or self.axes[0].set_xscale(xcale)
            or self.axes[0].set_yscale(yscale)
        )
...
num_epochs = 10
net1 = torch.nn.Sequential(torch.nn.Flatten(), torch.nn.Linear(num_inputs, num_outputs))
def init_weights(m):
...
train_ch3(net1, train_iter, test_iter, loss, num_epochs, optimizer)
